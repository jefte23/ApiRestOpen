...
@AllArgsConstructor
@NoArgsConstructor
public class HistoricoConsumoServicoMensal {
	
	public static final String HISTORICO_COM_CONSUMO_MENSAL_SERVICO = " SELECT " 
		...
		+ " FROM TAB_CSM_GR CS INNER JOIN TAB_GR GR ON CS.CD_GR=GR.CD_GR INNER JOIN TAB_GR_SRVC GS ON GR.CD_GR=GS.CD_GR INNER JOIN TAB_SRVC SV ON SV.CD_SRVC=GS.CD_SRVC WHERE CS.TIP_GR ='M' AND GR.NM_GR = :nomeGrupo ORDER BY  CS.DT_CSM_GR DESC";
